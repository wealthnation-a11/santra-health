...
-- per-user usage limit overrides
ALTER TABLE public.profiles
  ADD COLUMN IF NOT EXISTS daily_message_limit_override integer,
  ADD COLUMN IF NOT EXISTS monthly_voice_limit_override integer,
...
BEGIN
  ...
  VALUES (auth.uid(), 'update_profile', _user_id::text, jsonb_build_object(
    'full_name', _full_name, 'country', _country, 'state', _state,
    'phone', _phone, 'preferred_language', _preferred_language, 'gender', _gender
  ));
END $$;

-- 3. set per-user limit overrides
CREATE OR REPLACE FUNCTION public.admin_set_user_limits(
  _user_id uuid, _daily_messages integer, _monthly_voice integer
) RETURNS void LANGUAGE plpgsql SECURITY DEFINER SET search_path = public AS $$
BEGIN
  IF NOT public.has_role(auth.uid(), 'admin') THEN RAISE EXCEPTION 'Unauthorized'; END IF;
  UPDATE public.profiles SET
    daily_message_limit_override = _daily_messages,
    monthly_voice_limit_override = _monthly_voice,
    updated_at = now()
  WHERE id = _user_id;
  INSERT INTO public.admin_audit_log (admin_id, action, target_id, details)
  VALUES (auth.uid(), 'set_user_limits', _user_id::text,
    jsonb_build_object('daily', _daily_messages, 'voice', _monthly_voice));
END $$;

-- 4. expanded list with filters/sort/pagination
...
BEGIN
  IF NOT public.has_role(auth.uid(), 'admin') THEN RAISE EXCEPTION 'Unauthorized'; END IF;
  RETURN COALESCE((SELECT jsonb_agg(DISTINCT country) FROM public.profiles
                   WHERE country IS NOT NULL AND country <> ''), '[]'::jsonb);
END $$;

-- restrict execute
REVOKE ALL ON FUNCTION public.admin_user_detail(uuid) FROM PUBLIC, anon;
REVOKE ALL ON FUNCTION public.admin_update_profile(uuid,text,text,text,text,text,text,text) FROM PUBLIC, anon;
REVOKE ALL ON FUNCTION public.admin_set_user_limits(uuid,integer,integer) FROM PUBLIC, anon;
...
GRANT EXECUTE ON FUNCTION public.admin_set_user_limits(uuid,integer,integer) TO authenticated;
GRANT EXECUTE ON FUNCTION public.admin_list_users_v2(integer,integer,text,text,text,text,text,text) TO authenticated;
GRANT EXECUTE ON FUNCTION public.admin_user_countries() TO authenticated;
